...
export default function EditPatient({ navigation }) {
    /*
...
*/
    ...
    async function handleSubmit() {
        ...
        sessionUser.db_patient.map((detaList) => {if(detaList.db_name == data.name ) {
                console.log('tem um igual +++++++++++++++++++++++++++++++++++++++++')
        } else {
            console.log('não tem um igual +++++++++++++++++++++++++++++++++++++++++')
            console.log( detaList.db_name )
    ...
    async function get() {
        await AsyncStorage.getItem(`@teste@sessionkeypatient`)
            .then(req => JSON.parse(req))
            .then(json => [console.log('sucesso!'), patient =json])
            .catch(error => console.log('error!'));

            console.log(patient);
        
            setName(patient.db_name);
            setGenero(patient.db_genero);
            setDiag(patient.db_diag);
            setParentName(patient.db_parentName);
            setParentCall(patient.db_parentCall);
            setParentEmail(patient.db_parentEmail);
    }
        ...
        <SafeAreaView style={style.container}>
            ...
            <Appbar style={style.bottom}>
                <Appbar.Header style={style.appHeader}>
                    <Appbar.BackAction
                        onPress={navigateSession}
                    />
                    <Text style={style.title}>Edição de Paciente                           </Text>
                    <Appbar.Action icon="refresh" onPress={get}/>
                </Appbar.Header>
            </Appbar>
